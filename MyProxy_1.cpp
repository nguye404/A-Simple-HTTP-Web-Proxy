...
static int send_all(int& sock, char *buffer, long *len)
{
	long total = 0;
	int bytes_left = *len;
	int n;
...
string receive(int curSocket)
{
	int numsLeft = sizeof(long);
	long networkInt;
	char * length = (char*) &networkInt;
	
	// get the length of the string
	while(numsLeft)
	{
		int numReceived = recv(curSocket, length, numsLeft, 0);
		
		if(numReceived <= 0)
		{
			//COMMENT:	cout << "Error: String length receive failed." << endl; 
			//COMMENT:	exit (-1);
			return INTERNAL_ERROR;
		}
		numsLeft = numsLeft - numReceived;
		length = length + numReceived;
	}
	networkInt = ntohl(networkInt);
	
	int charsLeft = networkInt;
    ...
    while(charsLeft > 0)
    {
        ...
        if((size_recv =  recv(curSocket , chunk , CHUNK_SIZE , 0) ) < 0)
        {
            break;
        }
        else
        {
			...
			strReceive = strReceive + string(chunk);
        }
		charsLeft = charsLeft - CHUNK_SIZE;
    }
    return strReceive;
}
...
static void process(void* param) 
{
  ...
  int* telnet_sock = (int*) param;
	...
	string response = receive(web_server_sock);
	...
	long response_length = sizeof(response_char);
	if (send_all(*telnet_sock, response_char, &response_length) < 0) 
	{
		send_error(*telnet_sock);
		close(web_server_sock);
		return;
